var SkyRTC = function() {
    ...
    skyrtc.prototype.connect = function(server, room) {
        ...
        this.on('ready', function() {
            that.createPeerConnections();//创建与其他用户连接的PeerConnections
            that.addStreams();//将本地流添加到所有的PeerConnection实例中
            that.addDataChannels();//对所有的PeerConnections创建Data channel
        ...
        if (getUserMedia) {
            ...
            getUserMedia.call(navigator, options, function(stream) {
                    ...
                    if (that.initializedStreams === that.numStreams) {
                        that.emit("ready");//yan-getUserMedia调用成功后才能执行后续信令交互部分
                    }
                },
                function(error) {
        ...
        for (socketId in that.fileChannels) {
            for (sendId in that.fileChannels[socketId]) {
                if (that.fileChannels[socketId][sendId].state === "send") {
                    nextTick = true;
                    that.sendFileChunk(socketId, sendId);//yan-只发送指定文件的一个包
                }
            }
        }
        if (nextTick) {//yan-所有文件同时发送，若大文件则重发多次包
            setTimeout(function() {
                that.sendFileChunks();
            }, 10);
        }
